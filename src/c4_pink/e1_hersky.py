...
def sc_herworld(w: World):
    ...
    return w.scene("彼女の世界",
            ...
            w.comment("$autocarが事故っている映像を入れ込む"),
            asahi.come("$autocarの後部座席の車窓から、$Sはぼんやりと空を見上げた", "&"),
            outside.look("グレィ一色で、それは秋という季節を感じさせるには何もかも情報量が足りない"),
            outside.look("街は直線を重ねた味気ない建物しか存在せず、歩道や中央分離帯に申し訳程度の人工の緑が植えられている",
                "対抗車線を走るのは全て同じ形で色だけ異なる$autocarばかりだ", "当然運転席には誰も乗っていない",
                "$CSにとっては見慣れた光景だった"),
            ...
            _.do("クリーム色の扉の前に立つと音声ガイドが確認中だと伝える",
                ...
                "ただ入館証のみで歩けるよりずっとレベルの高いセキュリティエリアになるので、系列の会社の大掛かりな認証装置を持ってきたとは上司の$yoshiから聞かされている"),
...
def sc_simulation(w: World):
    ...
    return w.scene("シミュレーション",
            ...
            _.do("仕方なく途中経過のデータのスナップショットから復元し、再度シミュレーションし直すことにした"),
            _.do("スリープ状態から復帰した世界のデータには何も変化がない"),
            asahi.talk("問題なさそうね"),
            _.do("今回は今までよりもデータ項目を増やした上、$S独自のシミュレーションプログラムをそこで生活する人間の思考モデルとして取り入れていた",
            ...
            asahi.talk("昨日頼まれていたパンデミック時の医療リソースの逼迫状況のモデルと社会の混乱予測、あと$autocarが全てストップした際に発生する事故・事件予測のサンプル、",
                "上がってます"),
            yoshi.talk("本当は大手の$w_cyber社のような気象予測まで盛り込んだ大規模なシミュレーション実験をしたいものだが、この国の予算じゃ期待するだけ徒労だろうね"),
...
def sc_notice(w: World):
    ...
    return w.scene("気づき",
            ...
            _.explain("該当の人物は少年のようだ",
                "彼は一目散に川へと向かって駆けている",
                "どうやらその川に子猫のオブジェクトがいて、溺れているようなのだ",
                "小動物についてはごく簡単な思考アルゴリズムしか実装していないので危険を認識できずに落ちてしまっただけだろう",
                "放っておけば死んで消える",
                "彼はそれを助けようというのか躊躇することなく川に入ると、その子猫を持ち上げた",
                "しかし、その少年のオブジェクトは一秒ほど明滅して消えてしまう", "冬の川で溺れ死んだのだ"),
            _.do("数分前のスナップショットからやり直し、子猫のオブジェクトを川から排除する",
                "彼は特に急ぐことなく普通に歩道を歩いて橋の方へと向かった"),
            asahi.talk("何だったのかしら"),
            _.do("気になって時間を進めてみると、今度は女性のバッグを強引に盗もうとしている男に遭遇し、男を取り押さえようとして脇腹を刺され、再び彼は消えた",
            ...
            _.think("それが今目の前で起こったのは、明らかに自己犠牲だった",
                "明確なバグだ", "何かの設定をミスしたのだろうとデータやプログラムの不備を探そうとしたがどこにも見当たらない"),
